"""
Enhanced Markdown Generator V5
Generates daily update markdown with meta-summary and test mode support
"""
...
class MarkdownGeneratorV5:
    def __init__(self, template_path: str = None):
        self.template_path = template_path or "config/templates/daily-update-v5.md"
    ...
    def _get_default_template(self) -> str:
        """Default template with meta-summary and section order"""
...
{daily_stoicism_content}'''
    ...
    def _format_readwise_highlights(self, highlights: List[Dict[str, Any]]) -> str:
        """Format Readwise highlights section"""
        if not highlights:
            return ""

        content = "\n<div style='font-size: 1.3em; font-weight: bold; text-decoration: underline; margin-bottom: 0.5em;'>Readwise Highlights</div>\n\n"
        content += "*Recent passages you've highlighted*\n\n"

        for highlight in highlights:
            # Extract highlight data
            text = highlight.get('text', '')
            title = highlight.get('title', 'Unknown')
            author = highlight.get('author', 'Unknown Author')
            ...
            # Add note if present
            if note:
                content += f"**Your note:** {note}\n\n"

        content += "\n<hr style='border: none; border-top: 1px solid #ddd; margin: 1.5em 0;' />\n\n"
        return content

    def _format_readwise_reader(self, articles: List[Dict[str, Any]]) -> str:
        """Format Readwise Reader articles section"""
        if not articles:
            return ""

        content = "\n<div style='font-size: 1.3em; font-weight: bold; text-decoration: underline; margin-bottom: 0.5em;'>Readwise Reader</div>\n\n"
        content += "*Articles you've saved to read*\n\n"
        ...
        for article in articles:
            ...
            info_parts = []
            if word_count > 0:
                reading_time = word_count // 265  # ~265 words per minute
                info_parts.append(f"{reading_time} min read")
            if progress > 0:
                info_parts.append(f"{progress}% complete")
    ...
    def _format_meta_summary(self, meta_summary: str) -> str:
        """Format the AI-generated meta-summary section (200 words, 7-day analysis)"""
        if not meta_summary:
            return ""  # No content = no section at all

        content = f"\n<div style='font-size: 1.3em; font-weight: bold; text-decoration: underline; margin-bottom: 0.5em;'>Key Themes This Week</div>\n\n"
        content += meta_summary.strip() + "\n\n"
        content += "<hr style='border: none; border-top: 1px solid #ddd; margin: 1.5em 0;' />\n\n"
    ...
    def generate_daily_update(self,
        ...
        section_configs = {
            ...
            'good_news': {'title': 'Good News & Inspiration'},
            'ai_technology': {'title': 'AI & Technology'},
            'academia_research': {'title': 'Academia & Research'},
            'moonshot_strategy': {'title': 'Moonshot Strategy & Scientific Breakthroughs'},
            'market_business': {'title': 'Business & Markets'},
            'personal_finance_fire': {'title': 'Personal Finance'},
            'us_news': {'title': 'US News Brief'},
            'space_news': {'title': 'Space & Exploration'},
            'sustainable_homes': {'title': 'Sustainable Homes & Building'},
            'solopreneur_startups': {'title': 'Solopreneurs & Small Startups'},
        ...
        template_vars['meta_summary_content'] = self._format_meta_summary(
            supplementary_data.get('meta_summary', '')
        )

        template_vars['daily_stoicism_content'] = self._format_stoicism_quote(
            supplementary_data.get('stoicism_quote') or {}
        )

        template_vars['readwise_highlights_content'] = self._format_readwise_highlights(
            supplementary_data.get('readwise_highlights', [])
        )

        template_vars['readwise_reader_content'] = self._format_readwise_reader(
            supplementary_data.get('readwise_reader', [])
        )
        ...
        if not supplementary_data.get('meta_summary', ''):
            empty_sections.append('Key Themes This Week')
        stoicism_quote = supplementary_data.get('stoicism_quote') or {}
        if not stoicism_quote.get('quote'):
        ...
        try:
            ...
            # Add "No New Articles" section at the bottom if needed
            if empty_sections:
                # Create a formatted section listing categories with no articles
                empty_note = "\n\n<hr style='border: none; border-top: 1px solid #ddd; margin: 1.5em 0;' />\n\n"
                empty_note += "<div style='font-size: 1.1em; font-weight: bold; margin-bottom: 0.5em;'>No New Articles From:</div>\n\n"

                # List categories with no articles as bullet points
                for section in empty_sections:
                    empty_note += f"• {section}\n"

                markdown_content += empty_note
...
'''
    ...
    def save_to_file(self, content: str, output_dir: str, date: datetime = None, test_mode: bool = False) -> str:
        """Save markdown content to file"""
        if date is None:
            date = datetime.now()

        # Add TEST prefix to filename in test mode to avoid conflicts with production
        prefix = "TEST-" if test_mode else ""
        filename = f"{date.strftime('%Y-%m-%d')}-{prefix}daily-update.md"
        ...
        try:
            ...
            logger.info(f"Daily update V5 saved to: {filepath}")
            return filepath
