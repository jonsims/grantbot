#!/usr/bin/env python3
"""
Daily News Agent V4 - Advanced Features
Includes AI meta-summary, theme tracking, and performance optimizations
"""
...
from processors.ai_summarizer_v4 import AISummarizerV4
from processors.theme_tracker_v4 import ThemeTracker
from generators.markdown_v4 import MarkdownGeneratorV4
...
class DailyNewsAgentV4:
    def __init__(self, config_path: str = "config/sources-v4.yaml", use_cache: bool = True):
        """Initialize the V4 Daily News Agent with theme tracking"""
        ...
        self.ai_summarizer = AISummarizerV4(
            claude_api_key=os.getenv('CLAUDE_API_KEY'),
            openai_api_key=os.getenv('OPENAI_API_KEY')
        )
        self.markdown_generator = MarkdownGeneratorV4()
        ...
        logger.info(f"Daily News Agent V4 initialized - {self.version_manager.get_version_string()}")
    ...
    def collect_all_content(self) -> tuple[Dict[str, list], Dict[str, Any]]:
        ...
        # Add Readwise highlights and Reader articles
        if self.readwise_collector.enabled:
    ...
    def generate_daily_update(self, output_dir: str = None, test_mode: bool = False) -> str:
        """Generate complete V4 daily update with meta-summary"""
        mode_str = "TEST" if test_mode else "PRODUCTION"
        logger.info(f"=== Starting V4 Daily News Update Generation ({mode_str}) ===")
        ...
        try:
            ...
            # Step 3: Generate V4 markdown with version and test mode
            ...
            if self.email_sender.enabled:
                ...
                email_sent = self.email_sender.send_daily_update(
                    ...
                    version="v4"  # Clearly identify v4 emails
                )
                if email_sent:
                    logger.info(f"Daily update v4 emailed successfully ({source_label})")
...
def main():
    """Main entry point for V4"""
    import argparse

    parser = argparse.ArgumentParser(description='Generate V4 daily news update with meta-summary')
    parser.add_argument('--test', action='store_true', help='Generate test update (adds "Test" to title)')
    parser.add_argument('--test-components', action='store_true', help='Test all V4 components')
    parser.add_argument('--output', '-o', help='Output directory for generated file')
    parser.add_argument('--config', '-c', default='config/sources-v4.yaml', help='Config file path')

    args = parser.parse_args()

    try:
        agent = DailyNewsAgentV4(config_path=args.config)

        if args.test_components:
            results = agent.test_components_v2()
            print("\n=== V4 Component Test Results ===")
            for component, passed in results.items():
                status = "✓ PASS" if passed else "✗ FAIL"
                print(f"{component:25} {status}")

            if not all(results.values()):
                print("\n⚠️  Some V4 components failed. Check logs for details.")
                sys.exit(1)
            else:
                print("\n🎉 All V4 components working!")
        else:
            # Generate V4 daily update (test mode if --test flag is used)
            filepath = agent.generate_daily_update(args.output, test_mode=args.test)
            mode_str = "TEST" if args.test else "PRODUCTION"
            print(f"✅ V4 daily update generated ({mode_str}): {filepath}")
